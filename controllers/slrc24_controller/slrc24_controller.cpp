...
double AGVal[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
void ReadGsensors() { //function for read Ground sensors
 ...
 // std::cout<<"sensor"<<i<<"    "<<DGval[i]<< std::endl;
  ...
  AGVal[i] = 1 - ((lineSensorVal[i] - 256) / (992 - 256));
  }
}
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    //cout<<lineSensorVal[0]<<" "<<lineSensorVal[1]<<" "<<lineSensorVal[2]<<" "<<lineSensorVal[3]<<" "<<lineSensorVal[4]<<" "<<lineSensorVal[5]<<" "<<lineSensorVal[6]<<" "<<lineSensorVal[7]<<endl;
    cout<<AGVal[0]<<" "<<AGVal[1]<<" "<<AGVal[2]<<" "<<AGVal[3]<<" "<<AGVal[4]<<" "<<AGVal[5]<<" "<<AGVal[6]<<" "<<AGVal[7]<<endl;
